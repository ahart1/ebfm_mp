...
### Store Globe count for each simulation
GlobeCountFinal <- NULL
...
for(isim in 1:NumberSim){
  ...
  CatBioRow <- which(GlobeStoreTemp[,"MSY"]==Ecosystem_ResultingOptimalHRates[isim,"MSY"])
  Ecosystem_CatBio_UnderOptimalHRates[isim,] <- GlobeStoreTemp[CatBioRow,]
  
  GlobeCountFinal <- c(GlobeCountFinal, GlobeCount)
}

##########################################################
# Calculate ref points for ecosystem and aggregate groups
##########################################################
RefPtData_CatBio <- read.csv("EcosystemRefPointEstimate_CatBio.csv")
RefPtData_HRate <- read.csv("EcosystemRefPointEstimate_HRate.csv")

BioDataTemp <- cbind(RefPtData_CatBio[,11:20])
EcosystemRefPt <- rowSums(BioDataTemp)
hist(EcosystemRefPt)
mean(EcosystemRefPt)
median(EcosystemRefPt)
0.5*median(EcosystemRefPt) # This is the actual ref point used

AggRefPt <- rep(NA,4)

PiscivoresBioTemp <- RefPtData_CatBio[,c(11,15)] 
BenthivoresBioTemp <- RefPtData_CatBio[,c(12,18,19,20)] 
PlanktivoresBioTemp <- RefPtData_CatBio[,c(13,14)]  
ElasmobranchsBioTemp <- RefPtData_CatBio[,c(16,17)] 

AggRefPt[1] <- 0.5*median(rowSums(PiscivoresBioTemp))
AggRefPt[2] <- 0.5*median(rowSums(BenthivoresBioTemp))
AggRefPt[3] <- 0.5*median(rowSums(PlanktivoresBioTemp))
AggRefPt[4] <- 0.5*median(rowSums(ElasmobranchsBioTemp))

# AggRefPt[1] <- 0.5*median(RefPtData_CatBio[,"Biomass_GB_Cod"])
# AggRefPt[2] <- 0.5*median(RefPtData_CatBio[,"Biomass_GB_Haddock"])
# AggRefPt[3] <- 0.5*median(RefPtData_CatBio[,"Biomass_Herring"]) # Herring ref pt
# AggRefPt[4] <- 0.5*median(RefPtData_CatBio[,"Biomass_Mackerel"])
# AggRefPt[5] <- 0.5*median(RefPtData_CatBio[,"Biomass_Redfish"])
# AggRefPt[6] <- 0.5*median(RefPtData_CatBio[,"Biomass_Skates"])
# AggRefPt[7] <- 0.5*median(RefPtData_CatBio[,"Biomass_Spiny_dogfish"])
# AggRefPt[8] <- 0.5*median(RefPtData_CatBio[,"Biomass_GB_WinterFlounder"])
# AggRefPt[9] <- 0.5*median(RefPtData_CatBio[,"Biomass_GB_YellowtailFlounder"])
# AggRefPt[10] <- 0.5*median(RefPtData_CatBio[,"Biomass_GOM_GB_WindowpaneFlounder"])
