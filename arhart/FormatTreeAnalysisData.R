...
FormatTreeAnalysisData <- function(FileName=NULL, Nsim=NULL, CeilingValue=NULL, BMSY=NULL){
  ...
  for(i in 1:Nsim){
    ...
    Results[i,"FreqSSCollapse"] <- length(which((Biomass[nrow(Biomass),] < SpeciesBMSY[,"Bmsy"]*0.5)==TRUE))
    ###########
    ...
    # Average Total System Biomass # could I use mean(Biomass) instead==Yes?, system biomass=row sum for each year, want average over total number of years
...
Result_Ceiling50000 <- FormatTreeAnalysisData(FileName="sampleformat_results50000.json", Nsim=5, CeilingValue=50000, BMSY=SpeciesBMSY)
Result_Ceiling75000 <- FormatTreeAnalysisData(FileName="sampleformat_results75000.json", Nsim=5, CeilingValue=75000, BMSY=SpeciesBMSY)
Result_Ceiling100000 <- FormatTreeAnalysisData(FileName="sampleformat_results100000.json", Nsim=5, CeilingValue=100000, BMSY=SpeciesBMSY)
Result_Ceiling125000 <- FormatTreeAnalysisData(FileName="sampleformat_results125000.json", Nsim=5, CeilingValue=125000, BMSY=SpeciesBMSY)
Result_Ceiling150000 <- FormatTreeAnalysisData(FileName="sampleformat_results150000.json", Nsim=5, CeilingValue=150000, BMSY=SpeciesBMSY)
Result_Ceiling175000 <- FormatTreeAnalysisData(FileName="sampleformat_results175000.json", Nsim=5, CeilingValue=175000, BMSY=SpeciesBMSY)
Result_Ceiling200000 <- FormatTreeAnalysisData(FileName="sampleformat_results200000.json", Nsim=5, CeilingValue=200000, BMSY=SpeciesBMSY)
...
write.table(FormattedTreeData, file="FormattedTreeData_StatsTest")
